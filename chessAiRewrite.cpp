...
std::vector<std::array<int, 4>> getAllMoves(std::array<int, 69>& board, int color)
{
    auto t1 = std::chrono::high_resolution_clock::now();
    std::vector<std::array<int, 4>> allMoves;
    for (int i = 0; i < 64; i++)
    {
        int p = board[i];
        if (p != 0 && Game::getColor(p) == color)
        {
            std::vector<std::array<int, 2>> moves = Game::getMoves(board, Game::getX(p), Game::getY(p));
...
int miniMax(std::array<int, 69>& board, int depth, bool maximisingPlayer)
{
    ...
    {
        std::vector<std::array<int, 4>> allMoves = getAllMoves(board, Game::blackID);
        if (allMoves.size() == 0)
        {
        ...
        int value = INT_MIN;
        for (std::array<int, 4>& move : allMoves)
        {
            std::array<int, 69> newBoard = board;
            Game::movePiece(move[0], move[1], move[2], move[3], newBoard);
    ...
    {
        std::vector<std::array<int, 4>> allMoves = getAllMoves(board, Game::whiteID);
        if (allMoves.size() == 0)
        {
...
void makeBestMove(std::array<int, 69>& board, int color)
{
    numPositions = 0;
    auto t1 = std::chrono::high_resolution_clock::now();
    std::vector<std::array<int, 4>> allMoves = getAllMoves(board, color);
    ...
    std::array<int, 4> bestMove = allMoves[std::find(scores.begin(), scores.end(), value) - scores.begin()];
    Game::movePiece(bestMove[0], bestMove[1], bestMove[2], bestMove[3], board);
